 ...
 */
  ...
  /* Also notify PO - only for actual VFS calls, not for PM-internal pauses */
  {
	struct mproc *p_po;
      int i;
      ...
      for (i = 0; i < NR_PROCS; i++) {
          ...
          if ((p_po->mp_flags & IN_USE) && strcmp(p_po->mp_name, "po") == 0) {
              
              if (rmp->mp_endpoint == p_po->mp_endpoint) {
                  break; 
              }
              ...
              m_po.m1_i1 = rmp->mp_pid;  

              asynsend3(p_po->mp_endpoint, &m_po,AMF_NOREPLY);
